...
myFont :: String
myFont = "xft:FantasqueSansM Nerd Font:size=9:style=regular:antialias=true:hinting=true"
...
myLayout = mkToggle (NOBORDERS ?? NBFULL ?? EOT) . avoidStruts . lessBorders OnlyScreenFloat $ focusTracking layouts
  where
    ...
    layouts = dwindle ||| tiled ||| monocle ||| floating ||| tabbed'

    -- Layouts
    dwindle = rn "Dwindle" $ spacing myGaps $ Dwindle R CCW 1.5 1.1
    tiled = rn "Tiled" $ spacing myGaps $ dragWindows $ ResizableTall nmaster delta ratio []
    monocle = rn "Monocle" $ spacing myGaps Full
    floating = rn "Floating" positionStoreFloat
    tabbed' = rn "Tabbed" $ spacing' (myGaps + 5) $ tabbed shrinkText myTabConfig

    -- Simplify things
    rn n = renamed [Replace n]
    dragWindows = windowNavigation . draggingVisualizer
    ...
    spacing' :: Integer -> l a -> ModifiedLayout Spacing l a
    spacing' i = spacingRaw False (Border i i i i) True (Border 0 0 0 0) True
...
myAddKeys' =
  [ Title "Misc",
    ...
    Key ("M-S-r", spawn "xmonad --recompile && xmonad --restart", Desc "Recompile and restart Xmonad~/.config/gtk-4.0"),
    ...
    Key ("<XF86AudioLowerVolume>", spawn "wpctl set-volume @DEFAULT_AUDIO_SINK@ 4%-", Desc "Decrease the volume"),
    Key ("<XF86AudioRaiseVolume>", spawn "wpctl set-volume -l 1.0 @DEFAULT_AUDIO_SINK@ 4%+", Desc "Increase the volume"),
    ...
    Key ("M-<XF86MonBrightnessDown>", spawn "ddcutil setvcp 10 - 15", Desc "Decrease the multi-monitor brightness"),
    Key ("M-<XF86MonBrightnessUp>", spawn "ddcutil setvcp 10 + 15", Desc "Increase the multi-monitor brightness"),
    ...
    Key ("<XF86Favorites>", spawn myClipboardMenu, Desc "Open the clipboard menu"),
    Key ("S-<XF86Favorites>", spawn "rofi -modi emoji -show emoji", Desc "Open the emoji picker"),
...
myStartupHook = do
  ...
  spawnOnce "jamesdsp --tray" -- Audio effects
  ...
  spawnOnce "redshift-gtk" -- Night light / Auto brightness
  spawnOnce "kdeconnectd" -- KDE Connect Daemon
  spawnOnce "noisetorch -i -o" -- Noise Reduction (For Mic)
  spawnOnce "caffeine" -- Prevent screen from turning off
